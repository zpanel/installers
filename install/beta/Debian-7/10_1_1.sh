...
if [ "$OS" = "Debian" ] && [ "$VER" = "7" ] || [ "$VER" = "7.0" ] || [ "$VER" = "7.1" ] || [ "$VER" = "7.2" ] || [ "$VER" = "7.3" ] ; then
echo "Ok."
else
...
echo "mysql server 5.6 défault password in /root/.mysql_secret"
read -s -p "Enter Your current Password root of mysql : " password
